...
const generateEmailVerificationToken = asyncHandler(async (req, res) => {
    ...
    if (user.emailVerificationTokenExpiry && user.emailVerificationTokenExpiry > new Date()) {
        ...
        if (secondsRemaining > 540) {
            return res.status(429).json(new ApiError(429, `Please wait ${Math.ceil((600 - (600 - secondsRemaining)) / 60)} minute(s) before requesting a new code`));
        }
    }
    // Generate OTP
    const otp = generateSixDigitsOTP();
...
export { registerUser, loginUser, logoutUser, getCurrentUser, forgotPassword, resetPassword, generateEmailVerificationToken, verifyEmail, refreshAccessToken, };
